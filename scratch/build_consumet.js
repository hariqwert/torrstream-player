...
const scriptIndex = code.lastIndexOf('<script>');
if (scriptIndex === -1) {
  console.error('Could not find <script> tag in consumet.html');
  process.exit(1);
}

const htmlBody = code.substring(0, scriptIndex);

const fullScriptContent = `<script>
let lucideTimeout;
...
async function fetchAniListGraphQL(query, variables = {}) {
    try {
        ...
        console.warn('AniList GraphQL fetch error or timeout:', err);
        return null;
    }
}
...
// Global App State
let TMDB_KEY = "15d2ea6d0dc1d476efbca3eba2b9bbfb";
let activeTab = 'home';
...
async function initApp() {
    ...
    setTimeout(() => loadExtraHomeShelves(), 1100);
    setTimeout(() => loadSportsChannels(), 1200);
    setTimeout(() => { loadAnimeCatalog().then(() => { triggerLucide(); }); }, 1300);
...
async function refreshAllData(silent = false) {
    await Promise.all([
        ...
        loadSportsChannels().catch(() => {}),
        loadAnimeCatalog().catch(() => {})
    ]);
...
function switchTab(tab) {
    ...
    views.forEach(v => {
        ...
        if (tabBtn) {
            if (v === tab) {
                tabBtn.className = "nav-link active-tab text-[15px] font-bold text-white relative py-2 px-3 rounded-full hover:bg-white/5 transition-all flex items-center gap-2";
            } else {
                tabBtn.className = "nav-link text-[15px] font-semibold text-zinc-400 hover:text-white relative py-2 px-3 rounded-full hover:bg-white/5 transition-all flex items-center gap-2";
            }
        }
...
function appendFilterGrid(items, gridId, defaultType) {
    ...
    items.forEach(item => {
        ...
        card.innerHTML = \`
            <div class="relative aspect-[2/3] overflow-hidden bg-zinc-950">
                <img loading="lazy" src="\${poster}" alt="\${title}" class="w-full h-full object-cover md:group-hover:scale-110 transition-transform duration-500">
                <button onclick="event.stopPropagation(); openDetails('\${item.id}', '\${item.media_type || defaultType}')" title="View Info" class="absolute top-2.5 left-2.5 z-20 bg-black/80 hover:bg-red-600 active:scale-90 text-white px-2 py-1 rounded-full border border-white/20 shadow-xl flex items-center gap-1 backdrop-blur-md transition-all duration-200 hover:scale-105 active:bg-red-600">
                    <i data-lucide="info" class="w-3.5 h-3.5 text-amber-400"></i>
                    <span class="text-[9px] font-black uppercase tracking-wider text-white">Info</span>
                </button>
                <div class="absolute inset-0 bg-black/40 backdrop-blur-[2px] opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
            ...
            <div class="p-3.5 bg-zinc-950/90 flex-grow flex flex-col justify-between border-t border-white/5">
                ...
                <div class="flex items-center justify-between text-[9px] font-medium text-zinc-400 uppercase tracking-wider mt-1.5">
                    <span>\${year}</span>
                    <span class="text-white/80 bg-white/10 px-2 py-0.5 rounded-full border border-white/10">\${(item.media_type || defaultType) === 'tv' ? 'Series' : 'Movie'}</span>
                </div>
            </div>
        \`;
...
async function executeSearch() {
    ...
    try {
        ...
        resultsContainer.innerHTML = \`<div class="text-center py-8 text-red-400 text-xs font-bold uppercase">Search failed: \${err.message}</div>\`;
    }
}
...
async function openDetails(id, type = 'movie') {
    ...
    try {
        ...
        document.getElementById('detailType').textContent = type === 'tv' ? 'TV Series' : 'Movie';
...
function startActualFullscreenPlayback() {
    ...
    const type = selectedMedia.media_type || (selectedMedia.title ? 'movie' : 'tv');
...
function filterEpgGrid() {
    const query = document.getElementById('epgSearchInput').value.toLowerCase().trim();
    if (!query) {
        renderDemoEpgData();
        return;
    }
...
const finalCode = htmlBody + fullScriptContent;
fs.writeFileSync(filePath, finalCode, 'utf8');
console.log('Successfully updated consumet.html');
