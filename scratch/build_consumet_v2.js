...
// Key Rotation Pool for 100% TMDB Reliability
const TMDB_KEYS = [
    "9d83476d2e27f56748167514c69cd2b4",
    "15d2ea6d0dc1d476efbca3eba2b9bbfb",
    "8352514578b9b8b0931221b2fa3d37a2"
];
let tmdbKeyIdx = 0;
...
async function fetchTMDB(endpoint, params = {}) {
    for (let attempts = 0; attempts < TMDB_KEYS.length; attempts++) {
        ...
        // Rotate key on error / rate-limit
...
async function initApp() {
    ...
    // Staggered loading of all Home shelves to prevent rate limiting
    loadAllHomeShelves();
...
function switchTab(tab) {
    ...
    if (tab === 'home') {
        // Ensure Home data is loaded if shelves are empty
...
async function openDetails(id, type = 'movie') {
    ...
    try {
        ...
        // Fallback retry with opposite type if 404
        try {
...
console.log('Successfully updated consumet.html v2, size:', finalCode.length);
