(function() {
    ...
    async function checkStatus() {
        try {
            ...
            if (window.location.pathname.indexOf('hari.html') !== -1) {
                return;
            }
    ...
    if (_origFetch) {
        window.fetch = async function() {
            try {
                ...
                if (res && res.status === 503 && !is503Page && window.location.pathname.indexOf('hari.html') === -1) {
                    window.location.reload();
                }
                return res;
            } catch(err) {
    ...
    if (_origXSend) {
        XMLHttpRequest.prototype.send = function() {
            this.addEventListener('load', function() {
                if (this.status === 503 && !is503Page && window.location.pathname.indexOf('hari.html') === -1) {
