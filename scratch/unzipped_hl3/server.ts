...
app.use((req, res, next) => {
    ...
    const isClientPage = ['/', '/index.php', '/play.php', '/play_consumet.php', '/play_torrent.php', '/playlist.php', '/hero.html', '/consumet.html', '/books.html', '/music.html', '/play_media.html', '/sample_maintenance.html'].includes(req.path);
...
app.get('/hari.html', requireAdmin, (req, res) => {
...
app.get('/api/music/yt-search', async (req: Request, res: Response) => {
    try {
        ...
        const r = await yts(query);
        const videos = (r && r.videos && r.videos.length > 0) ? r.videos : ((r && r.all) ? r.all.filter((x: any) => x && (x.type === 'video' || x.videoId)) : []);
...
app.get('/play_torrent.php', (req, res) => {
    const stream_url = (req.query.url as string) || (req.query.id as string) || '';
    const name = (req.query.name as string) || 'Torrent Stream';
    
    servePhpFile(path.join(process.cwd(), 'play_torrent.php'), res, {
        '<?php echo htmlspecialchars($name); ?>': name,
        '<?php echo htmlspecialchars($stream_url); ?>': stream_url,
        '{{STREAM_URL}}': stream_url,
        '{{NAME}}': name
...
app.get('/api/epg-json', async (req: Request, res: Response) => {
    ...
    try {
        ...
        for (const url of urls) {
            try {
                ...
                while((pm = progRegex.exec(xmlText)) !== null) {
                    ...
                    const catM = inner.match(/<category[^>]*>([^<]+)<\/category>/);
                    allProgrammes.push({
                        channel: pm[3],
                        start: pm[1],
                        end: pm[2],
                        ...
                        desc: descM ? descM[1] : "",
                        category: catM ? catM[1] : "General"
                    });
                }
            } catch (err: any) {
                console.error(`Failed to parse EPG feed ${url}: ${err.message}`);
            }
        }
        
        if (allChannels.length === 0) {
            return res.status(500).json({ error: 'No channels found in provided EPG sources' });
        }
        ...
        return res.status(500).json({ error: `Failed to process EPG JSON: ${err.message}` });
    }
});
