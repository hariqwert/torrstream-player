...
// 1. Create public/watchdog.js
const watchdogCode = `(function() {
...
fs.writeFileSync('public/watchdog.js', watchdogCode, 'utf8');
console.log('Created public/watchdog.js');

// 2. Script Tag to inject
const scriptTag = `<script src="/watchdog.js" id="maintenance-watchdog"></script>`;

// 3. Clean server.ts
let serverCode = fs.readFileSync('server.ts', 'utf8');

// Replace any corrupted inline script blocks
serverCode = serverCode.replace(/<script id="maintenance-watchdog">[\s\S]*?<\/script>/g, scriptTag);

// Ensure /watchdog.js is bypassed in firewall
if (!serverCode.includes("req.path === '/watchdog.js'")) {
    serverCode = serverCode.replace(
        /req\.path === '\/api\/system\/public-status' \|\|/g,
...
filesToPatch.forEach(file => {
    if (fs.existsSync(file)) {
        ...
        content = content.replace(/<script id="maintenance-watchdog">[\s\S]*?<\/script>/g, scriptTag);
        if (!content.includes('src="/watchdog.js"')) {
        ...
        console.log(`Updated ${file} with /watchdog.js tag`);
    }
});

console.log('Watchdog setup complete');
