...
function fetchUrl(url: string, referer: string = 'https://dlhd.st/'): Promise<string> {
...
async function resolveChannelStream(rawId: string): Promise<string | null> {
    ...
    let html = await fetchUrl(`https://hamis.romponalis.st/premiumtv/daddy3.php?id=${encodeURIComponent(channelId)}`, 'https://dlhd.st/');
    let streamUrl = decodeStreamUrl(html);
    ...
    if (!streamUrl && !isDlhd) {
        html = await fetchUrl(`https://hux-giants.shop/embed/${encodeURIComponent(channelId)}`, 'https://timstreams.st/');
        streamUrl = decodeStreamUrl(html);
    }
...
export async function getOrUpdatePlaylist(force: boolean = false): Promise<string> {
    ...
    if (!data || !Array.isArray(data) || data.length === 0) {
        try {
            ...
            const dlhdLogoMap: Record<string, string> = {
            ...
            try {
                ...
                if (dlhdRes) {
                    ...
                    if (Array.isArray(rawDlhd)) {
                        dlhdChannels = rawDlhd.map((c: any) => {
                        ...
                        }).filter((c: any) => Boolean(c.channel_id) && !c.channel_id.includes('undefined') && allowedDlhdIds.has(c.raw_id))
                          .map(({ raw_id, ...c }: any) => c);
                    }
                }
            } catch (e: any) {
            ...
            if (data.length > 0) {
                fs.writeFileSync(channelsPath, JSON.stringify(data, null, 2), 'utf-8');
...
router.get('/api/sports/channels', (req: Request, res: Response) => {
    try {
        const channelsPath = path.join(process.cwd(), 'assets', 'channels.json');
        if (fs.existsSync(channelsPath)) {
