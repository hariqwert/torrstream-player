...
exports.activeStreamingResponses = void 0;
exports.isSafeUrl = isSafeUrl;
exports.killAllActiveStreams = killAllActiveStreams;
exports.streamUrl = streamUrl;
exports.clearManifestCache = clearManifestCache;
exports.handleLiveStream = handleLiveStream;
...
async function isSafeUrl(urlStr) {
    try {
        ...
        if (ipParts.length !== 4) {
            if (address.includes(':')) {
                return address !== '::1' && !address.startsWith('fe80:') && !address.startsWith('fc00:') && !address.startsWith('fd00:');
            }
            return false;
        }
        // Private IP Ranges
...
function resolveM3u8Url(base, pathStr) {
...
exports.activeStreamingResponses = new Set();
function killAllActiveStreams() {
    console.log(`[PROXY] Terminating ${exports.activeStreamingResponses.size} active streams immediately.`);
    for (const res of exports.activeStreamingResponses) {
        try {
            res.destroy();
        }
        catch (e) { }
    }
    exports.activeStreamingResponses.clear();
}
async function streamUrl(url, headersArray, req, res) {
    console.log("STREAM_URL_FETCH:", url);
    // Check System Power State (Proxy Kill)
    if (server_1.systemState.status === 'offline' || server_1.systemState.status === 'killed') {
    ...
    if ((0, server_1.shouldSendUserIp)(cleanIp)) {
        headers['X-Forwarded-For'] = cleanIp;
        headers['X-Real-IP'] = cleanIp;
    }
    (0, server_1.addStreamingIp)(cleanIp);
    exports.activeStreamingResponses.add(res);
    res.on('close', () => {
        (0, server_1.removeStreamingIp)(cleanIp);
        exports.activeStreamingResponses.delete(res);
    });
    if (url.includes('hiveatick') || url.includes('casadenoval') || url.includes('timstreams') || url.includes('hux-giants')) {
        headers['Referer'] = 'https://hux-giants.shop/';
        headers['Origin'] = 'https://hux-giants.shop/';
    }
    ...
    try {
        ...
        const proxyReq = client.request(url, requestOptions, (proxyRes) => {
            ...
            if (!contentType || contentType === 'application/octet-stream' || contentType.includes('text/plain') || contentType.includes('text/html')) {
                ...
                else if (lowerUrl.includes('.ts') || lowerUrl.includes('/play/') || lowerUrl.includes('/live/'))
...
async function handleLiveStream(req, res) {
    ...
    if (req_id && req_id.includes('hux-giants.shop/embed/')) {
        ...
        if (cached && Date.now() - cached.time < 5 * 1000) {
            ...
            try {
                ...
                const embedResponse = await axios_1.default.get(req_id, {
                    headers: { 'Referer': 'https://hux-giants.shop/' }
                });
    ...
    if (m3u_mode) {
        ...
        if (!urlToCheck && wanda)
            urlToCheck = stalkerAPI_1.StalkerAPI.scarletWitch('decrypt', wanda);
        if (!urlToCheck && cassie)
            urlToCheck = stalkerAPI_1.StalkerAPI.scarletWitch('decrypt', cassie);
        ...
        else if (urlToCheck && (urlToCheck.includes('timstreams.st') || urlToCheck.includes('casadenoval.uk') || urlToCheck.includes('hux-giants.shop') || urlToCheck.includes('hiveatick'))) {
            headersArray.push('Referer: https://hux-giants.shop/');
            headersArray.push('Origin: https://hux-giants.shop/');
        }
    }
    else if (portal) {
        if (portal.Model)
            headersArray.push(`X-User-Agent: Model: ${portal.Model}; Link: WiFi`);
        if (portal.URL)
            headersArray.push(`Referer: ${portal.URL}/c/`);
        if (portal.MAC)
            headersArray.push(`Cookie: mac=${portal.MAC}; stb_lang=en; timezone=GMT`);
        ...
        const legacyTokenFile = path_1.default.join(process.cwd(), 'doctor_strange', 'token.stalker');
        let tokenToUse = '';
        ...
        if (!tokenToUse && fs_1.default.existsSync(legacyTokenFile)) {
            try {
                const tokenData = JSON.parse(fs_1.default.readFileSync(legacyTokenFile, 'utf8'));
                tokenToUse = tokenData?.STALKER?.Token;
            }
            catch (e) { }
        }
    ...
    let custom_ua = req.query['http-user-agent'] || '';
    if (custom_ua) {
    ...
    if (req_id) {
        ...
        server_1.activeSessions.set(cleanIp, {
            ip: cleanIp,
            channelId: req_id,
            startTime: Date.now(),
        ...
        if (req_id.startsWith('http://') || req_id.startsWith('https://')) {
            ...
            if (dlhdId && dlhdId !== 'undefined') {
                try {
                    ...
                    const daddyRes = await axios_1.default.get(daddyUrl, {
                        headers: {
                            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                    ...
                    if (!resolved) {
                        ...
                        const fallbackRes = await axios_1.default.get(fallbackDlhdUrl, {
                            headers: {
                                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                                'Referer': 'https://dlhd.st/'
                            },
            ...
            if (req.query.nocache) {
                const cacheFile = path_1.default.join(process.cwd(), 'cache_stalker', `${req_id}.json`);
                if (fs_1.default.existsSync(cacheFile))
                    fs_1.default.unlinkSync(cacheFile);
            }
            const apiResStr = await stalkerAPI_1.StalkerAPI.doctor_strange(req_id, portal);
            try {
            ...
            if (apiRes?.STALKER?.data && typeof apiRes.STALKER.data === 'string' && apiRes.STALKER.data.includes('Authorization failed')) {
                const tokenStalkerPath = path_1.default.join(process.cwd(), 'doctor_strange', 'token.stalker');
                if (fs_1.default.existsSync(tokenStalkerPath))
                    fs_1.default.unlinkSync(tokenStalkerPath);
                return res.redirect(req.originalUrl);
            }
            stream = apiRes?.STALKER?.cmd || '';
            stream = stalkerAPI_1.StalkerAPI.id_generator(stream);
        }
        if (!stream) {
            try {
                const embedRes = await axios_1.default.get(`https://hux-giants.shop/embed/${encodeURIComponent(req_id)}`, {
                    headers: {
                        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        ...
        if (!stream) {
            try {
                const daddyRes = await axios_1.default.get(`https://hamis.romponalis.st/premiumtv/daddy3.php?id=${encodeURIComponent(req_id)}`, {
                    headers: {
                        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                        'Referer': 'https://dlhd.st/'
                    },
        ...
        if (stream.includes('hux-giants.shop/embed/')) {
            if (!await isSafeUrl(stream)) {
                return res.status(403).send('Access Denied: Unsafe URL detected.');
            }
            try {
                const embedRes = await axios_1.default.get(stream, {
                    headers: {
                        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                        'Referer': 'https://hux-giants.shop/'
                    }
        ...
        if (stream.startsWith('http')) {
            ...
            let stalkerRes = await stalkerAPI_1.StalkerAPI.stalkerRequest(stream, headersArray, 'GET', null, true);
            ...
            if (status_code !== 200 && is_cached) {
                ...
                if (fs_1.default.existsSync(cacheFile))
                    fs_1.default.unlinkSync(cacheFile);
                const freshResStr = await stalkerAPI_1.StalkerAPI.doctor_strange(req_id, portal);
                let freshRes = null;
                try {
                ...
                if (stream.startsWith('http')) {
                    stalkerRes = await stalkerAPI_1.StalkerAPI.stalkerRequest(stream, headersArray, 'GET', null, true);
            ...
            if ((status_code === 401 || status_code === 403 || (typeof content === 'string' && content.includes('Authorization failed'))) && portal) {
                ...
                const freshProfileStr = await stalkerAPI_1.StalkerAPI.doctor_strange('', portal);
                let freshToken = '';
                try {
                ...
                if (freshToken) {
                    ...
                    const freshResStr = await stalkerAPI_1.StalkerAPI.doctor_strange(req_id, portal);
                    let freshResObj = null;
                    try {
                        freshResObj = JSON.parse(freshResStr);
                    }
                    catch (e) { }
                    const newCmd = freshResObj?.STALKER?.cmd || stream;
                    if (newCmd && newCmd.startsWith('http')) {
                        stream = stalkerAPI_1.StalkerAPI.id_generator(newCmd);
                        stalkerRes = await stalkerAPI_1.StalkerAPI.stalkerRequest(stream, headersArray, 'GET', null, true);
                        content = stalkerRes.STALKER.data;
                        status_code = stalkerRes.STALKER.info.http_code;
                        const final_url = stalkerRes.STALKER.info.final_url || stream;
                    ...
                    for (let line of lines) {
                        ...
                        if (line.includes('URI="')) {
                            ...
                            if (uriMatch) {
                                ...
                                let stalkerEnc = stalkerAPI_1.StalkerAPI.scarletWitch('encrypt', finalUri.substring(0, finalUri.lastIndexOf('/') + 1));
                                let paramEnc = stalkerAPI_1.StalkerAPI.scarletWitch('encrypt', finalUri);
                            ...
                            if (url && (url.startsWith('http://') || url.startsWith('https://'))) {
                                ...
                                let finalUrl = url;
                                let stalkerEnc = stalkerAPI_1.StalkerAPI.scarletWitch('encrypt', finalUrl.substring(0, finalUrl.lastIndexOf('/') + 1));
                                let paramEnc = stalkerAPI_1.StalkerAPI.scarletWitch('encrypt', finalUrl);
    ...
    if (wanda && token === 'STALKER_PRO') {
        let wanda_url = stalkerAPI_1.StalkerAPI.scarletWitch('decrypt', wanda);
        let stalkerRes = await stalkerAPI_1.StalkerAPI.stalkerRequest(wanda_url, headersArray, 'GET', null, true);
        let content = stalkerRes.STALKER.data;
        let status_code = stalkerRes.STALKER.info.http_code;
        ...
        while ([301, 302, 303, 307, 308].includes(status_code) && redirectCount < 10) {
            ...
            if (redirectUrl) {
                ...
                stalkerRes = await stalkerAPI_1.StalkerAPI.stalkerRequest(wanda_url, headersArray, 'GET', null, true);
                content = stalkerRes.STALKER.data;
                status_code = stalkerRes.STALKER.info.http_code;
                const final_wanda_url = stalkerRes.STALKER.info.final_url || wanda_url;
                wanda_url = final_wanda_url;
    ...
    if (cassie && token === 'STALKER_PRO') {
        res.setHeader('Access-Control-Allow-Origin', '*');
        const decrypted_cassie = stalkerAPI_1.StalkerAPI.scarletWitch('decrypt', cassie);
        try {
            const u = new URL(decrypted_cassie);
            const ext = path_1.default.extname(u.pathname).toLowerCase();
            if (ext === '.m4s' || ext === '.mp4') {
