 ...
 */
function sanitizeUserAgent(uaInput) {
    if (!uaInput)
 ...
 */
async function isSafeStreamUrl(urlStr) {
    try {
        ...
        return await (0, proxy_1.isSafeUrl)(urlStr);
    }
    catch {
        return false;
    }
...
router.options('/api/stream-proxy', (req, res) => {
...
router.options('/api/internal-stream', (req, res) => {
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Access-Control-Allow-Methods', 'GET, HEAD, OPTIONS');
    res.setHeader('Access-Control-Allow-Headers', 'Range, Content-Type, User-Agent, Referer');
    res.sendStatus(204);
 ...
 */
function streamWithRedirects(targetUrl, headers, res, req, redirectCount = 0) {
    if (redirectCount > 5) {
    ...
    const clientReq = requestModule.get(targetUrl, {
        ...
        if (upstream.statusCode && upstream.statusCode >= 300 && upstream.statusCode < 400 && upstream.headers.location) {
            const redirectUrl = new url_1.URL(upstream.headers.location, targetUrl).toString();
 ...
 */
router.get('/api/internal-stream', async (req, res) => {
    const urlStr = (req.query.url || '').trim();
    const referer = (req.query.referer || '').trim();
    ...
    res.on('close', () => (0, server_1.removeStreamingIp)(cleanIp));
...
router.get('/api/stream-proxy', async (req, res) => {
    ...
    const isHls = !isXtreamPlay && (lowerUrl.includes('.m3u8') ||
    ...
    if (mode === 'copy') {
        ...
        args.push('-c', 'copy', '-max_muxing_queue_size', '2048', '-f', 'mpegts', '-');
    }
    else if (mode === 'transcode') {
        // Full CPU x264 re-encode for non-H264 video streams
        args.push('-c:v', 'libx264', '-preset', 'ultrafast', '-tune', 'zerolatency', '-pix_fmt', 'yuv420p', '-g', '50', '-b:v', '1500k', '-maxrate', '2000k', '-bufsize', '4000k', '-c:a', 'aac', '-ac', '2', '-ar', '48000', '-b:a', '128k', '-af', 'aresample=async=1000', '-max_muxing_queue_size', '2048', '-f', 'mpegts', '-');
    }
    else {
        // Default 'remux': Video stream copy (0% CPU, full 60fps) + Audio convert to AAC 2-channel 48kHz
        args.push('-c:v', 'copy', '-c:a', 'aac', '-ac', '2', '-ar', '48000', '-b:a', '128k', '-af', 'aresample=async=1000', '-max_muxing_queue_size', '2048', '-f', 'mpegts', '-');
    }
    res.setHeader('Content-Type', 'video/mp2t');
    (0, server_1.addStreamingIp)(cleanIp);
    const onDisconnectProxy = () => (0, server_1.removeStreamingIp)(cleanIp);
    ...
    function killFFmpegProcess(proc, reason) {
        if (!proc || proc.killed)
            return;
