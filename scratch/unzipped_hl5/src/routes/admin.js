...
const sportsM3u_1 = require("./sportsM3u");
...
const M3U_VAULT = path_1.default.join(DARK_SIDE, 'm3u_playlists');
const DB_FILE = path_1.default.join(DARK_SIDE, 'admin_db.json');
...
function getDB() {
    if (!fs_1.default.existsSync(DB_FILE)) {
        ...
        fs_1.default.writeFileSync(DB_FILE, JSON.stringify(initial, null, 4), 'utf8');
        return initial;
    }
    try {
        const data = JSON.parse(fs_1.default.readFileSync(DB_FILE, 'utf8'));
        ...
        if ((!data.portals || data.portals.length === 0)) {
            ...
            fs_1.default.writeFileSync(DB_FILE, JSON.stringify(data, null, 4), 'utf8');
        }
        return {
...
function saveDB(db) {
    fs_1.default.writeFileSync(DB_FILE, JSON.stringify(db, null, 4), 'utf8');
}
/**
 * Helper to scan existing portal json files in doctor_strange.
 */
function scanExistingStalkerPortals() {
    const portals = [];
    ...
    if (fs_1.default.existsSync(loginFile)) {
        try {
            const activeLogin = JSON.parse(fs_1.default.readFileSync(loginFile, 'utf8'));
            if (activeLogin?.URL) {
...
const storage = multer_1.default.diskStorage({
    ...
    filename: (req, file, cb) => {
        const hash = crypto_1.default.createHash('md5').update(file.originalname + Date.now()).digest('hex').substring(0, 8);
        const ext = path_1.default.extname(file.originalname).toLowerCase();
...
router.get('/features', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    res.json({
        status: "success",
...
router.post('/features', auth_1.requireAdmin, (req, res) => {
    const { m3uEnabled, stalkerEnabled, firewallEnabled, publicPlaylistEnabled } = req.body;
    const db = getDB();
    if (db.features) {
        if (m3uEnabled !== undefined)
            db.features.m3uEnabled = !!m3uEnabled;
        if (stalkerEnabled !== undefined)
            db.features.stalkerEnabled = !!stalkerEnabled;
        if (firewallEnabled !== undefined)
            db.features.firewallEnabled = !!firewallEnabled;
        if (publicPlaylistEnabled !== undefined)
            db.features.publicPlaylistEnabled = !!publicPlaylistEnabled;
    }
    else {
        db.features = {
...
router.get('/stats', auth_1.requireAdmin, (req, res) => {
    ...
    db.m3uPlaylists.forEach(pl => {
        const channelsFile = path_1.default.join(DARK_SIDE, `m3u_channels_${pl.id}.json`);
        if (fs_1.default.existsSync(channelsFile)) {
            try {
                const channels = JSON.parse(fs_1.default.readFileSync(channelsFile, 'utf8'));
    ...
    res.json({
        ...
        stats: {
            ...
            activeSessions: server_1.activeSessions.size,
...
router.post('/system/clear-cache', auth_1.requireAdmin, (req, res) => {
    const cacheDir = path_1.default.join(process.cwd(), 'cache_stalker');
...
router.post('/system/cleanup', auth_1.requireAdmin, (req, res) => {
    try {
        const type = req.body.type;
        const cacheDir = path_1.default.join(process.cwd(), 'cache_stalker');
        const dataDir = path_1.default.join(process.cwd(), 'doctor_strange');
        if (type === 'cache') {
            if (fs_1.default.existsSync(cacheDir)) {
                const files = fs_1.default.readdirSync(cacheDir);
                for (const file of files) {
                    if (file !== '.htaccess' && file !== 'index.php') {
                        const fullPath = path_1.default.join(cacheDir, file);
            ...
            if (fs_1.default.existsSync(dataDir)) {
                ...
                for (const file of files) {
                    if (file.startsWith('recents_') && file.endsWith('.json')) {
                        fs_1.default.unlinkSync(path_1.default.join(dataDir, file));
                    }
                }
            }
...
router.get('/system/status', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    res.json({
        status: "success",
        systemStatus: db.systemStatus || 'active'
...
router.get('/developer', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    res.json({
        status: "success",
...
router.post('/developer', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    const { developerMode } = req.body;
    db.developerMode = !!developerMode;
    saveDB(db);
    (0, server_1.refreshBlacklist)();
...
router.post('/developer/whitelist', auth_1.requireAdmin, (req, res) => {
    const { ip } = req.body;
    if (!ip)
        return res.status(400).json({ status: "error", message: "IP required" });
    const db = getDB();
    if (!db.allowedDeveloperIps)
        db.allowedDeveloperIps = [];
...
router.delete('/developer/whitelist', auth_1.requireAdmin, (req, res) => {
    const { ip } = req.body;
    if (!ip)
        return res.status(400).json({ status: "error", message: "IP required" });
    const db = getDB();
...
router.get('/maintenance', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    res.json({
        status: "success",
...
router.post('/maintenance', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    ...
    saveDB(db);
    server_1.systemState.maintenanceMode = db.maintenanceMode;
    server_1.systemState.activeTemplate = db.activeTemplate;
    server_1.systemState.maintenanceMusicMode = db.maintenanceMusicMode || 'query';
    server_1.systemState.maintenanceMusicQuery = db.maintenanceMusicQuery || 'lofi relax';
    server_1.systemState.maintenanceMusicPlaylist = db.maintenanceMusicPlaylist || [];
    server_1.systemState.consumetMaintenance = db.consumetMaintenance || false;
    server_1.systemState.playMaintenance = db.playMaintenance || false;
    server_1.systemState.playConsumetMaintenance = db.playConsumetMaintenance || false;
    // Clear sessions if entering maintenance
    if (db.maintenanceMode) {
        server_1.activeSessions.clear();
        const liveStalkerPath = path_1.default.join(process.cwd(), 'doctor_strange', 'live.stalker');
        if (fs_1.default.existsSync(liveStalkerPath))
            fs_1.default.unlinkSync(liveStalkerPath);
        (0, proxy_1.killAllActiveStreams)();
    }
    res.json({
        status: "success",
...
router.post('/system/toggle', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    ...
    server_1.systemState.status = nextStatus;
    // Force update exported variable if possible
    (0, server_1.refreshBlacklist)();
    if (nextStatus === "offline" || nextStatus === "killed") {
        server_1.activeSessions.clear();
...
router.get('/sessions', auth_1.requireAdmin, (req, res) => {
    const sessions = Array.from(server_1.activeSessions.values());
    res.json({
        status: "success",
        sessions: sessions
    });
...
router.post('/sessions/terminate', auth_1.requireAdmin, (req, res) => {
    const { ip } = req.body;
    if (!ip)
        return res.status(400).json({ status: "error", message: "IP required" });
    server_1.activeSessions.delete(ip);
    res.json({
        status: "success",
...
router.get('/blacklist', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    res.json({
        status: "success",
...
router.post('/blacklist', auth_1.requireAdmin, (req, res) => {
    const { ip } = req.body;
    if (!ip)
        return res.status(400).json({ status: "error", message: "IP required" });
    const db = getDB();
...
router.delete('/blacklist/:ip', auth_1.requireAdmin, (req, res) => {
    const { ip } = req.params;
    if (!ip)
        return res.status(400).json({ status: "error", message: "IP required" });
    const db = getDB();
...
router.get('/incidents', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    res.json({
        status: "success",
...
router.post('/login', (req, res) => {
    ...
    try {
        isPasswordCorrect = bcryptjs_1.default.compareSync(cleanPassword, adminPassHash);
    }
    catch (e) {
    ...
    if (userMatches && isPasswordCorrect) {
        const clientIp = (0, auth_1.getClientIp)(req);
        console.log(`[AUTH-SUCCESS] User "${cleanUsername}" logged in from ${clientIp}`);
        // Reset failures on success
        loginFailures.delete(clientIp);
        const token = jsonwebtoken_1.default.sign({ ip: clientIp, role: 'admin', user: adminUser }, auth_1.JWT_SECRET, { expiresIn: '12h' });
        ...
        if (attempts >= 5) {
            ...
            if (!db.blacklist.includes(clientIp)) {
                ...
                if (db.incidents.length > 50)
                    db.incidents = db.incidents.slice(0, 50);
                saveDB(db);
                (0, server_1.refreshBlacklist)();
                loginFailures.delete(clientIp);
 ...
 */
router.get('/heartbeat', auth_1.requireAdmin, (req, res) => {
    const clientIp = (0, auth_1.getClientIp)(req);
    return res.json({
        status: "success",
        ip: clientIp,
 ...
 */
router.get('/portals', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    const portals = db.portals.map(p => {
        const portalFile = path_1.default.join(DARK_SIDE, `${p.id}.json`);
        let extra = {};
        if (fs_1.default.existsSync(portalFile)) {
            try {
                extra = JSON.parse(fs_1.default.readFileSync(portalFile, 'utf8'));
            }
            catch (e) { }
        }
        return {
...
router.post('/portals', auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        ...
        if (isXtream) {
            ...
            try {
                ...
                const response = await axios_1.default.get(`${cleanUrl}/player_api.php?username=${username.trim()}&password=${password.trim()}`, { timeout: 8000 });
            ...
            fs_1.default.writeFileSync(path_1.default.join(DARK_SIDE, `${portalId}.json`), JSON.stringify(xtreamConfig, null, 4), 'utf8');
            if (newPortal.isDefault) {
                fs_1.default.writeFileSync(path_1.default.join(DARK_SIDE, 'login.stalker'), JSON.stringify(xtreamConfig, null, 4), 'utf8');
            }
            ...
            const testSn = sn ? sn.trim() : crypto_1.default.createHash('md5').update(mac.trim()).digest('hex').substring(0, 13).toUpperCase();
            const testConfig = {
                URL: cleanUrl,
                MAC: mac.trim(),
                SN: testSn,
                Model: (model || 'MAG250').trim(),
                D1: (device_id1 && device_id1.trim()) ? device_id1.trim() : crypto_1.default.createHash('sha256').update(mac.trim() + "D1").digest('hex').toUpperCase(),
                D2: (device_id2 && device_id2.trim()) ? device_id2.trim() : crypto_1.default.createHash('sha256').update(mac.trim() + "D2").digest('hex').toUpperCase(),
                user_agent: user_agent ? user_agent.trim() : "",
                image_version: image_version ? image_version.trim() : "218",
                Proxy: "AUTO",
                API: "263",
                Share: "OFF",
                hw_version: "1.7-BD-" + crypto_1.default.createHash('md5').update(mac.trim()).digest('hex').substring(0, 2).toUpperCase()
            };
            try {
                ...
                const handshakeRes = await stalkerAPI_1.StalkerAPI.handshake(testConfig, token ? token.trim() : '');
            ...
            const stalkerConfig = {
                ...testConfig,
                type: 'stalker',
                SG: signature ? signature.trim() : crypto_1.default.createHash('md5').update(mac.trim() + "SG").digest('hex').substring(0, 16).toUpperCase(),
                Token: token ? token.trim() : "",
                hw_version_2: crypto_1.default.createHash('md5').update((testSn + mac.trim()).toLowerCase()).digest('hex'),
                Name: newPortal.name
            };
            fs_1.default.writeFileSync(path_1.default.join(DARK_SIDE, `${portalId}.json`), JSON.stringify(stalkerConfig, null, 4), 'utf8');
            if (newPortal.isDefault) {
                fs_1.default.writeFileSync(path_1.default.join(DARK_SIDE, 'login.stalker'), JSON.stringify(stalkerConfig, null, 4), 'utf8');
            }
...
router.put('/portals/:id', auth_1.requireAdmin, async (req, res) => {
    const { id } = req.params;
    const { name, url, mac, model, sn, device_id1, device_id2, signature, image_version, token, user_agent, type, username, password } = req.body;
    const db = getDB();
    ...
    try {
        ...
        const portalFile = path_1.default.join(DARK_SIDE, `${id}.json`);
        ...
        if (isXtream) {
            ...
            try {
                ...
                const response = await axios_1.default.get(`${cleanUrl}/player_api.php?username=${newUsername}&password=${newPassword}`, { timeout: 8000 });
                const data = response.data;
                if (!data || data.user_info?.auth === 0) {
                    return res.status(400).json({ status: "error", message: "Handshake Failed: Invalid Xtream Username or Password." });
                }
            ...
            fs_1.default.writeFileSync(portalFile, JSON.stringify(xtreamConfig, null, 4), 'utf8');
            ...
            if (fs_1.default.existsSync(portalFile)) {
                try {
                    existingConfig = JSON.parse(fs_1.default.readFileSync(portalFile, 'utf8'));
                }
                catch (e) { }
            }
            const newMac = mac ? mac.trim() : (db.portals[idx].mac || '');
            const newSn = (sn !== undefined) ? (sn.trim() || crypto_1.default.createHash('md5').update(newMac).digest('hex').substring(0, 13).toUpperCase()) : (existingConfig.SN || crypto_1.default.createHash('md5').update(newMac).digest('hex').substring(0, 13).toUpperCase());
            const testConfig = {
                URL: cleanUrl,
                MAC: newMac,
                SN: newSn,
                Model: model ? model.trim() : (db.portals[idx].model || 'MAG250'),
                D1: (device_id1 !== undefined) ? (device_id1.trim() || crypto_1.default.createHash('sha256').update(newMac + "D1").digest('hex').toUpperCase()) : (existingConfig.D1 || crypto_1.default.createHash('sha256').update(newMac + "D1").digest('hex').toUpperCase()),
                D2: (device_id2 !== undefined) ? (device_id2.trim() || crypto_1.default.createHash('sha256').update(newMac + "D2").digest('hex').toUpperCase()) : (existingConfig.D2 || crypto_1.default.createHash('sha256').update(newMac + "D2").digest('hex').toUpperCase()),
                ...
                API: "263",
                Share: "OFF",
                hw_version: "1.7-BD-" + crypto_1.default.createHash('md5').update(newMac).digest('hex').substring(0, 2).toUpperCase()
            };
            try {
                console.log(`[HANDSHAKE] Testing Portal: ${testConfig.URL} with MAC: ${testConfig.MAC}, SN: ${testConfig.SN}, D1: ${testConfig.D1}, D2: ${testConfig.D2}`);
                const handshakeRes = await stalkerAPI_1.StalkerAPI.handshake(testConfig, token ? token.trim() : '');
            ...
            const updatedConfig = {
                ...
                D1: testConfig.D1,
                D2: testConfig.D2,
                SG: (signature !== undefined) ? (signature.trim() || crypto_1.default.createHash('md5').update(newMac + "SG").digest('hex').substring(0, 16).toUpperCase()) : (existingConfig.SG || crypto_1.default.createHash('md5').update(newMac + "SG").digest('hex').substring(0, 16).toUpperCase()),
                image_version: testConfig.image_version,
                Token: token ? token.trim() : (existingConfig.Token || ""),
                user_agent: testConfig.user_agent
            };
            fs_1.default.writeFileSync(portalFile, JSON.stringify(updatedConfig, null, 4), 'utf8');
            if (db.portals[idx].isDefault) {
                fs_1.default.writeFileSync(path_1.default.join(DARK_SIDE, 'login.stalker'), JSON.stringify(updatedConfig, null, 4), 'utf8');
                const tokenFile = path_1.default.join(DARK_SIDE, "token.stalker");
                const liveFile = path_1.default.join(DARK_SIDE, "live.stalker");
                const genreFile = path_1.default.join(DARK_SIDE, "genre.json");
                if (fs_1.default.existsSync(tokenFile))
...
router.post('/portals/:id/default', auth_1.requireAdmin, (req, res) => {
    const { id } = req.params;
    const db = getDB();
    ...
    const portalFile = path_1.default.join(DARK_SIDE, `${id}.json`);
    if (fs_1.default.existsSync(portalFile)) {
        const content = fs_1.default.readFileSync(portalFile, 'utf8');
        fs_1.default.writeFileSync(path_1.default.join(DARK_SIDE, 'login.stalker'), content, 'utf8');
        fs_1.default.writeFileSync(path_1.default.join(DARK_SIDE, 'active_playlist_id.txt'), 'portal', 'utf8');
...
router.delete('/portals/:id', auth_1.requireAdmin, (req, res) => {
    const { id } = req.params;
    const db = getDB();
    const idx = db.portals.findIndex(p => p.id === id);
    if (idx === -1) {
    ...
    const portalFile = path_1.default.join(DARK_SIDE, `${id}.json`);
    if (fs_1.default.existsSync(portalFile))
        fs_1.default.unlinkSync(portalFile);
    ...
    if (wasDefault) {
        ...
        if (db.portals.length > 0) {
            ...
            saveDB(db);
            const nextPortalFile = path_1.default.join(DARK_SIDE, `${db.portals[0].id}.json`);
            if (fs_1.default.existsSync(nextPortalFile)) {
                fs_1.default.writeFileSync(loginFile, fs_1.default.readFileSync(nextPortalFile));
            }
        }
    }
 ...
 */
router.get('/sports', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    return res.json({ status: "success", sports: db.sports || [] });
});
router.post('/sports', auth_1.requireAdmin, (req, res) => {
...
router.put('/sports/:id', auth_1.requireAdmin, (req, res) => {
    const { id } = req.params;
    const { title, icon, url } = req.body;
    if (!title || !url) {
        return res.status(400).json({ status: "error", message: "Missing required parameters: title and url are required." });
...
router.delete('/sports/:id', auth_1.requireAdmin, (req, res) => {
    const { id } = req.params;
    const db = getDB();
    if (!db.sports)
        db.sports = [];
...
router.post('/sports/extract', auth_1.requireAdmin, async (req, res) => {
    try {
        await (0, sportsM3u_1.getOrUpdatePlaylist)(true);
...
router.get('/sports/check', auth_1.requireAdmin, async (req, res) => {
    const url = req.query.url;
    if (!url) {
        return res.status(400).json({ status: "error", message: "Missing URL query parameter" });
    }
    try {
        const checkRes = await (0, axios_1.default)({
            method: 'get',
            url: url,
            timeout: 6000,
            headers: {
 ...
 */
router.get('/live_events', auth_1.requireAdmin, (req, res) => {
    const db = getDB();
    return res.json({ status: "success", liveEvents: db.liveEvents || [] });
});
router.post('/live_events', auth_1.requireAdmin, (req, res) => {
    const { title, icon, url } = req.body;
    if (!title || !url) {
        return res.status(400).json({ status: "error", message: "Missing required parameters: title and url are required." });
    }
...
router.put('/live_events/:id', auth_1.requireAdmin, (req, res) => {
    const { id } = req.params;
    const { title, icon, url } = req.body;
    if (!title || !url) {
        return res.status(400).json({ status: "error", message: "Missing required parameters: title and url are required." });
    ...
    if (!db.liveEvents)
        db.liveEvents = [];
    const itemIdx = db.liveEvents.findIndex((s) => s.id === id);
    if (itemIdx === -1) {
...
router.delete('/live_events/:id', auth_1.requireAdmin, (req, res) => {
    const { id } = req.params;
    const db = getDB();
    if (!db.liveEvents)
        db.liveEvents = [];
