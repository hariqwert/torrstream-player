...
import { JWT_SECRET, requireAdmin } from './src/middleware/auth';
import { StalkerAPI, StalkerConfig } from './src/stalkerAPI';
import { handleLiveStream } from './src/proxy';
...
app.use((req, res, next) => {
    ...
    console.log(`[ACCESS CHECK] Path: ${req.path}, Status: ${systemState.status}, Maintenance: ${systemState.maintenanceMode}, DevMode: ${systemState.developerMode}`);

    // 2.5 DEVELOPER MODE: Block non-whitelisted IPs
    if (systemState.developerMode && !isAdminPath && !isLoginAction && req.path !== '/hari.html' && req.path !== '/hari.js' && req.path !== '/watchdog.js' && req.path !== '/api/system/public-status') {
    ...
    if (systemState.status === 'offline' || systemState.status === 'killed' || systemState.maintenanceMode || isSpecificMaintenance) {
        ...
        const title = systemState.maintenanceMode ? "Maintenance Mode" : "System Offline";
        const description = systemState.maintenanceMode 
        ...
        const iconColor = systemState.maintenanceMode ? "text-amber-500" : "text-red-500";
        const iconBg = systemState.maintenanceMode ? "bg-amber-500/10 border-amber-500/20" : "bg-red-500/10 border-red-500/20";
...
<body class="text-white min-h-screen flex flex-col items-center justify-center p-4 sm:p-6 text-center select-none">
    ...
    <div class="max-w-md w-full z-10 relative space-y-6">
        ...
        <div class="flex items-center justify-center gap-2.5 bg-white/5 border border-white/5 py-2 px-4 rounded-full w-fit mx-auto backdrop-blur-md">
            <span class="w-2 h-2 rounded-full ${systemState.maintenanceMode ? 'bg-amber-500 animate-pulse' : 'bg-red-500 animate-pulse'}"></span>
...
const requireAdminHtml = (req: Request, res: Response, next: any) => {
    const cookieHeader = req.headers.cookie || '';
...
app.get('/hari.html', requireAdminHtml, (req, res) => {
    ...
    res.sendFile(path.join(process.cwd(), 'public', 'hari.html'));
});

app.get('/hari.js', requireAdmin, (req, res) => {
    res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
    res.sendFile(path.join(process.cwd(), 'public', 'hari.js'));
});
...
app.get('/api/music/yt-info', async (req: Request, res: Response) => {
    try {
        ...
        const ytArgs = ['-J', '--no-playlist', '--no-warnings', '--js-runtimes', 'node'];
        ...
        child.on('close', (code: number | null) => {
            if (code === 0 && stdout) {
                try {
                    ...
                    return res.json({
                        ...
                        thumbnail: info.thumbnail || (info.thumbnails && info.thumbnails.length > 0 ? info.thumbnails[info.thumbnails.length - 1].url : `https://i.ytimg.com/vi/${vParam}/hqdefault.jpg`),
                        video_qualities: ['1080p', '720p', '480p', '360p', '240p'],
                        audio_qualities: ['320kbps', '256kbps', '128kbps', 'm4a']
                    });
                } catch (e) {
            ...
            res.json({
                ...
                title: 'YouTube Video',
                uploader: 'YouTube',
                thumbnail: `https://i.ytimg.com/vi/${vParam}/hqdefault.jpg`,
                video_qualities: ['1080p', '720p', '480p', '360p', '240p'],
                audio_qualities: ['320kbps', '256kbps', '128kbps', 'm4a']
            });
        });
...
app.get('/api/music/download-youtube', async (req: Request, res: Response) => {
    try {
        const vParam = (req.query.v || req.query.url || req.query.id || '').toString().trim();
        const type = (req.query.type || 'video').toString().toLowerCase().trim(); // 'video' or 'audio'
        const quality = (req.query.quality || (type === 'video' ? '720p' : '320kbps')).toString().toLowerCase().trim();
        const customTitle = (req.query.title || '').toString().trim();

        if (!vParam) {
            return res.status(400).json({ error: 'YouTube video ID or URL is required (param v or url)' });
        }

        const rawVideoId = vParam.replace(/^(https?:\/\/)?(www\.)?(youtube\.com\/watch\?v=|youtu\.be\/)/, '').split('&')[0];
        const ytUrl = `https://www.youtube.com/watch?v=${rawVideoId}`;

        const ytdlpPath = path.join(process.cwd(), 'bin', 'yt-dlp');
        const tmpDir = path.join(process.cwd(), 'doctor_strange', 'tmp_downloads');
        if (!fs.existsSync(tmpDir)) {
            fs.mkdirSync(tmpDir, { recursive: true });
        }

        const safeTitle = (customTitle || 'youtube_track').replace(/[^a-zA-Z0-9_\-\s]/g, '').trim().replace(/\s+/g, '_') || 'youtube_download';
        const fileExt = type === 'video' ? 'mp4' : (quality === 'm4a' ? 'm4a' : 'mp3');
        const filename = `${safeTitle}_${quality}.${fileExt}`;
        const tmpFilePath = path.join(tmpDir, `yt_${Date.now()}_${crypto.randomBytes(4).toString('hex')}.${fileExt}`);

        const ytdlpArgs: string[] = ['--no-warnings', '--js-runtimes', 'node'];

        // Check for YouTube cookie file
        const possibleCookiePaths = [
            path.join(process.cwd(), 'doctor_strange', 'youtube_cookies.txt'),
            path.join(process.cwd(), 'youtube_cookies.txt')
        ];
        ...
        if (type === 'video') {
            let maxH = '720';
            if (quality.includes('1080')) maxH = '1080';
            else if (quality.includes('720')) maxH = '720';
            else if (quality.includes('480')) maxH = '480';
            else if (quality.includes('360')) maxH = '360';
            else if (quality.includes('240')) maxH = '240';

            ytdlpArgs.push('-f', `bestvideo[height<=${maxH}][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=${maxH}]+bestaudio/best[height<=${maxH}]/best`);
            ytdlpArgs.push('--merge-output-format', 'mp4');
        } else {
            if (quality === 'm4a') {
                ytdlpArgs.push('-f', 'bestaudio[ext=m4a]/bestaudio');
            } else {
                let qVal = '0'; // 320kbps equivalent
                if (quality.includes('256')) qVal = '2';
                else if (quality.includes('128')) qVal = '5';
                
                ytdlpArgs.push('-x', '--audio-format', 'mp3', '--audio-quality', qVal);
            }
        }

        ytdlpArgs.push('-o', tmpFilePath);
        ytdlpArgs.push(ytUrl);

        console.log(`[YouTubeDownload] Processing download: type=${type}, quality=${quality}, filename=${filename}`);
            ...
            if (code === 0 && fs.existsSync(tmpFilePath)) {
                ...
                if (!res.headersSent) {
                    ...
                    res.status(isBotBlock ? 403 : 500).json({ 
                        error: isBotBlock 
                            ? 'YouTube bot verification required for this video.' 
                            : 'Failed to process YouTube download request', 
                        bot_verification_required: isBotBlock,
                        video_id: rawVideoId,
                        details: errData.slice(-300) 
                    });
                }
            }
        });
        ...
        console.error('[YouTubeDownload] Server exception:', err);
        if (!res.headersSent) {
            res.status(500).json({ error: 'YouTube download server error: ' + err.message });
        }
    }
});
...
app.get('/api/live_events', (req: Request, res: Response) => {
    try {
...
// Search
app.get('/api/consumet/search', async (req: Request, res: Response) => {
    const query = req.query.q as string;
    ...
    try {
        ...
        try {
            ...
            const tmdbRes = await axios.get(`https://api.themoviedb.org/3/search/multi?api_key=${tmdbKey}&query=${encodeURIComponent(query)}`);
            if (tmdbRes.data && tmdbRes.data.results) {
                const mappedResults = tmdbRes.data.results.map((r: any) => ({
                    id: r.id.toString(),
...
export function serveAdminPasswordGate(res: Response) {
...
app.get('/play.php', (req, res) => {
    const id = (req.query.id as string) || '';
    const name = (req.query.name as string) || 'Live Channel';
    ...
    servePhpFile(path.join(process.cwd(), 'play.php'), res, {
        ...
        '{{SOURCE}}': (req.query.source as string) || 'index.php',
        '{{NAME}}': name,
...
app.get('/play_consumet.php', (req, res) => {
    const stream_url = (req.query.url as string) || (req.query.id as string) || '';
    const name = (req.query.name as string) || 'Live Stream';
...
app.all('/stalker_api.php', async (req: Request, res: Response) => {
    ...
    if (action === "admin_login") {
        ...
        if (verifyAdminPassword(password)) {
            ...
            const token = jwt.sign({ role: 'admin', ip: currentIp }, JWT_SECRET, { expiresIn: '30d' });
            ...
            if (channels.length === 0 || mediaType === 'sports' || mediaType === 'dlhd') {
                try {
                    ...
                    if (fs.existsSync(channelsPath)) {
                        const localData = JSON.parse(fs.readFileSync(channelsPath, 'utf-8'));
