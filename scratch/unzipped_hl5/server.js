...
const requireAdminHtml = (req, res, next) => {
    const cookieHeader = req.headers.cookie || '';
...
app.get('/api/music/yt-info', async (req, res) => {
    try {
        ...
        const ytdlpPath = path_1.default.join(process.cwd(), 'bin', 'yt-dlp');
        const ytArgs = ['-J', '--no-playlist', '--no-warnings', '--js-runtimes', 'node'];
        const possibleCookiePaths = [
            path_1.default.join(process.cwd(), 'doctor_strange', 'youtube_cookies.txt'),
            path_1.default.join(process.cwd(), 'youtube_cookies.txt')
        ];
        ...
        ytArgs.push(ytUrl);
        const child = (0, child_process_1.spawn)(ytdlpPath, ytArgs);
        let stdout = '';
        let stderr = '';
...
app.get('/api/music/download-youtube', async (req, res) => {
    try {
        const vParam = (req.query.v || req.query.url || req.query.id || '').toString().trim();
        ...
        const ytdlpPath = path_1.default.join(process.cwd(), 'bin', 'yt-dlp');
        const tmpDir = path_1.default.join(process.cwd(), 'doctor_strange', 'tmp_downloads');
        ...
        const tmpFilePath = path_1.default.join(tmpDir, `yt_${Date.now()}_${crypto_1.default.randomBytes(4).toString('hex')}.${fileExt}`);
        const ytdlpArgs = ['--no-warnings', '--js-runtimes', 'node'];
        ...
        const child = (0, child_process_1.spawn)(ytdlpPath, ytdlpArgs);
        let errData = '';
        ...
        child.on('close', (code) => {
            if (isAborted)
                return;
            if (code === 0 && fs_1.default.existsSync(tmpFilePath)) {
                const stats = fs_1.default.statSync(tmpFilePath);
                ...
                const readStream = fs_1.default.createReadStream(tmpFilePath);
                readStream.pipe(res);
                readStream.on('end', () => {
                    setTimeout(() => {
                        if (fs_1.default.existsSync(tmpFilePath)) {
...
app.get('/api/music/proxy', async (req, res) => {
    ...
    try {
        ...
        if (req.query.download === '1') {
            const title = req.query.title ? req.query.title : 'audio';
