        ...
        async function initApp() {
            ...
            loadHomeSpotlight().then(() => {
                ...
                setTimeout(() => loadAsianDramaAndAnime(),
                loadHollywoodPremieres(),
                loadRegionalHits('ta', 'tamilShelf'),
                loadRegionalHits('ml', 'malayalamShelf'), 800);
        ...
        function renderTop10Shelf(items, elementId, badgePrefix = 'TODAY') {
            ...
            items.slice(0, 10).forEach((item, index) => {
                ...
                const poster = item.poster_path ? `https://image.tmdb.org/t/p/w500${item.poster_path}` : (item.images?.jpg?.large_image_url || 'https://images.unsplash.com/photo-1594909122845-11baa439b7bf?q=80&w=300&h=450&fit=crop');
                const rating = item.vote_average ? item.vote_average.toFixed(1) : (item.score ? item.score.toFixed(1) : 'NR');
            ...
            try {
                ...
                if (epgMatches.length > 0) {
                    ...
                    epgMatches.forEach(match => {
                        ...
                        htmlOutput += `
                        <div class="flex items-center gap-3 p-3 bg-emerald-950/20 hover:bg-emerald-900/40 border border-emerald-500/10 hover:border-emerald-500/30 rounded-xl cursor-pointer transition-all transform active:scale-[0.99] group" 
    ...
    if ('IntersectionObserver' in window) {
            ...
            items.slice(0, 15).forEach(item => {
                ...
                card.innerHTML = `
                    <div class="relative aspect-[2/3] overflow-hidden bg-zinc-950">
                        ...
                        <div class="absolute top-2.5 left-2.5 bg-indigo-500/80 backdrop-blur-md text-white px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider border border-indigo-400/30 flex items-center gap-1 shadow-md">
        ...
        async function renderEpgBoard() {
            ...
            days.forEach(day => {
                ...
                btn.className = `px-3.5 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider shrink-0 transition-all border ${isActive ? 'bg-emerald-600 text-white border-emerald-500 shadow-md shadow-amber-600/15' : 'bg-black/60 text-slate-400 hover:text-white border-white/5'}`;
            ...
            window.renderNextEpgChunk = async function() {
                ...
                sItem.innerHTML = `
                    <div class="w-9 h-9 bg-zinc-900 border border-white/10 rounded-xl overflow-hidden flex items-center justify-center shrink-0">
                        <img src="${chan.logo}" alt="" class="w-full h-full object-cover" onerror="this.outerHTML='<span class=&quot;text-[10px] font-bold text-emerald-400&quot;>${initials}</span>'">
                    </div>
                    <div class="min-w-0 flex-grow">
                        <p class="text-xs font-black uppercase text-white truncate leading-tight group-hover:text-emerald-400 transition-colors">${chan.name}</p>
                        <p class="text-[9px] font-semibold text-emerald-400 tracking-wider flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><i data-lucide="play" class="w-2.5 h-2.5"></i> Play Channel</p>
                    </div>
                `;
                ...
                chanProgs.forEach((prog, pIdx) => {
                    ...
                    const card = document.createElement('div');
                    card.className = "absolute top-[6px] bottom-[6px] bg-[#18181b]/90 hover:bg-emerald-500/20 hover:border-emerald-500 border border-white/10 rounded-lg px-3 py-1.5 cursor-pointer flex flex-col justify-center transition-all select-none overflow-hidden group shadow-md";
                    ...
                    card.innerHTML = `
                        <p class="epg-program-title text-[11px] font-extrabold uppercase tracking-wide text-zinc-100 group-hover:text-emerald-400 transition-colors truncate leading-tight">${prog.title}</p>
                        <div class="flex items-center justify-between text-[8px] font-bold text-zinc-400 uppercase tracking-widest mt-1">
                            <span class="truncate">${timeLabel}</span>
                            <span class="bg-black/30 px-1.5 py-0.5 rounded text-emerald-400 border border-white/5">${prog.category || "General"}</span>
                        </div>
                    `;
