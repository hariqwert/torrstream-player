                ...
                return `
                    <div class="bg-zinc-950/80 border border-white/10 hover:border-emerald-500/50 rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-xl hover:shadow-emerald-500/10 hover:-translate-y-1 relative group flex flex-col justify-between p-3.5" onclick="openFullscreenPlayer('${rawUrl}', '${encodeURIComponent(title).replace(/'/g, "%27")}')">
                        ...
                        <div class="flex items-center justify-between gap-1 mb-2.5">
                            ...
                            <button onclick="toggleSavedSport(event, '${encodeURIComponent(title).replace(/'/g, "%27")}', '${encodeURIComponent(rawUrl).replace(/'/g, "%27")}', '${encodeURIComponent(ch.logo || "").replace(/'/g, "%27")}')" class="w-6.5 h-6.5 rounded-full bg-black/60 hover:bg-black/80 border border-white/10 flex items-center justify-center transition-all active:scale-90">
                        ...
                        <div class="relative aspect-[16/10] bg-gradient-to-b from-zinc-900 to-black/80 rounded-xl p-2.5 flex items-center justify-center border border-white/5 mb-3 group-hover:border-emerald-500/30 transition-all overflow-hidden">
                            ...
                            <!-- Hover Play Overlay -->
                            <div class="absolute inset-0 bg-emerald-950/60 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
            ...
            container.innerHTML = `
                ...
                <div class="absolute bottom-0 left-0 p-8 sm:p-14 max-w-2xl z-20 animate-slide-up">
                    ...
                    <div class="flex flex-wrap items-center gap-4">
                        <button onclick="openFullscreenPlayer('${item.url}', '${encodeURIComponent(title)}')" class="px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-black uppercase tracking-widest rounded-xl transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.5)] hover:-translate-y-1 flex items-center gap-2 transform active:scale-95">
            ...
            itemsToRender.forEach(ch => {
                ...
                card.innerHTML = `
                    <div class="relative aspect-[2/3] overflow-hidden bg-zinc-900 flex items-center justify-center p-2">
                        ...
                        <button onclick="toggleSavedSport(event, '${encodeURIComponent(ch.name).replace(/'/g, "%27")}', '${encodeURIComponent(ch.url || ch.stream_url || "").replace(/'/g, "%27")}', '${encodeURIComponent(ch.logo || "").replace(/'/g, "%27")}')" class="absolute top-2 right-2 z-20 w-6 h-6 rounded-full bg-black/60 hover:bg-black/80 border border-white/10 flex items-center justify-center transition-all active:scale-90 opacity-0 group-hover:opacity-100">
        ...
        function renderSportsGrid(channels) {
            ...
            channels.forEach(ch => {
                ...
                card.innerHTML = `
                    <div class="relative aspect-[2/3] overflow-hidden bg-zinc-900 flex items-center justify-center p-2">
                        <img src="${poster}" alt="${title}" referrerpolicy="no-referrer" onerror="this.src='${fallbackLogo}'" class="max-w-full max-h-full object-contain filter drop-shadow-lg group-hover:scale-105 transition-transform duration-500">
                        <button onclick="toggleSavedSport(event, '${encodeURIComponent(ch.name).replace(/'/g, "%27")}', '${encodeURIComponent(ch.url).replace(/'/g, "%27")}', '${encodeURIComponent(ch.logo || "").replace(/'/g, "%27")}')" class="absolute top-2 right-2 z-20 w-6 h-6 rounded-full bg-black/60 hover:bg-black/80 border border-white/10 flex items-center justify-center transition-all active:scale-90 opacity-0 group-hover:opacity-100">
        ...
        function renderContinueWatchingHistory() {
            ...
            itemsToRender.forEach(item => {
                ...
                const actionBtnHtml = isDummy 
                    ? `<button onclick="event.stopPropagation(); showToast('Added ${title.replace(/'/g, "\\'")} to Watchlist!');" class="absolute top-2 right-2 z-20 w-7 h-7 rounded-full bg-indigo-600 hover:bg-indigo-700 border border-white/10 flex items-center justify-center text-white transition-all shadow-md active:scale-90 opacity-0 group-hover:opacity-100">
                           <i data-lucide="plus" class="w-3.5 h-3.5"></i>
                       </button>`
                    : `<button onclick="event.stopPropagation(); removeHistoryItem('${item.id}');" class="absolute top-2 right-2 z-20 w-7 h-7 rounded-full bg-black/60 hover:bg-red-600 border border-white/10 flex items-center justify-center text-zinc-400 hover:text-white transition-all shadow-md active:scale-90 opacity-0 group-hover:opacity-100">
        ...
        async function executeSearch() {
            ...
            resultsDiv.innerHTML = '<div class="text-center py-10 text-zinc-500 text-xs font-bold uppercase tracking-widest animate-pulse">Searching Cinema Catalogs...</div>';
            ...
            try {
                ...
                if (filtered.length === 0) {
                    resultsDiv.innerHTML = '<div class="text-center py-10 text-slate-500 text-xs font-bold uppercase tracking-widest">No matching cinema found</div>';
                    return;
                }
                ...
                filtered.forEach(item => {
                    ...
                    row.className = "flex items-center gap-4 p-3 bg-zinc-950/60 hover:bg-zinc-900 border border-white/5 hover:border-red-500/20 rounded-2xl cursor-pointer transition-all transform active:scale-[0.99] group";
                    row.onclick = () => {
                        closeSearchPalette();
                        openDetails(item.id, item.media_type);
                    };
                    ...
                    row.innerHTML = `
                        ...
                        <div class="min-w-0 text-left flex-grow">
                            ...
                            <div class="text-sm font-extrabold text-white truncate group-hover:text-red-500 transition-colors mt-0.5" title="${title}">${title}</div>
...
async function renderTimelines(containerId, timelinesArray) {
    ...
    for (const tl of timelinesArray) {
        ...
        async function fetchAndParseEpg() {
            ...
            try {
                ...
                if (feedVal === 'demo') {
                    ...
                    window.epgChannels = data.channels;
                    ...
                    // Fetch using proxy endpoint to strip CORS blockages
                    const response = await fetch(`/api/epg-proxy?url=${encodeURIComponent(url)}`);
                    if (!response.ok) {
                    ...
                    for (let i = 0; i < channelNodes.length; i++) {
                        ...
                        const name = nameNode ? nameNode.textContent : (id || "Unknown Channel");
                        
                        const iconNode = node.getElementsByTagName('icon')[0];
                        const logo = iconNode ? iconNode.getAttribute('src') : `https://images.unsplash.com/photo-1594909122845-11baa439b7bf?q=80&w=100&h=100&fit=crop`;
                        
                        if (id) {
                            parsedChannels.push({ id, name, logo });
                        }
                    }
                    ...
                    for (let i = 0; i < progNodes.length; i++) {
                        ...
                        const category = catNode ? catNode.textContent : "General";
                    ...
                    if (parsedChannels.length === 0) {
                        throw new Error("No channels found in XMLTV EPG data. Check if URL is valid.");
                    }

                    window.epgChannels = parsedChannels;
                    window.epgProgrammes = parsedProgrammes;
                }
        ...
        function renderEpgBoard() {
            ...
            channelCountText.textContent = window.epgChannels.length;

            window.epgChannels.forEach(chan => {
                // Sidebar Item
                const sItem = document.createElement('div');
                sItem.id = `epg-sidebar-item-${chan.id}`;
                sItem.className = "epg-sidebar-item h-[65px] px-4 flex items-center gap-3 bg-zinc-950 hover:bg-zinc-900/40 border-b border-white/5 transition-all";
                ...
                sItem.innerHTML = `
                    ...
                    <div class="min-w-0 flex-grow">
                        <p class="text-xs font-black uppercase text-white truncate leading-tight">${chan.name}</p>
                        <p class="text-[9px] font-semibold text-slate-500 tracking-wider">XMLTV Channel</p>
                    </div>
                `;
                ...
                const chanProgs = window.epgProgrammes.filter(p => p.channel === chan.id);
                ...
                chanProgs.forEach((prog, pIdx) => {
                    ...
                    card.onclick = () => openEpgProgramDetails(prog, chan.name);
        ...
        function openEpgProgramDetails(prog, channelName) {
        ...
        function filterEpgGrid() {
            ...
            const rows = document.querySelectorAll('.epg-timeline-row');
            
            rows.forEach(row => {
                const chanId = row.dataset.channelId;
                const chanName = row.dataset.channelName.toLowerCase();
                
                let match = chanName.includes(q);
                if (!match) {
                    const programTitles = Array.from(row.querySelectorAll('.epg-program-title')).map(p => p.textContent.toLowerCase());
                    match = programTitles.some(title => title.includes(q));
                }
                
                const sidebarItem = document.getElementById(`epg-sidebar-item-${chanId}`);
                if (match) {
